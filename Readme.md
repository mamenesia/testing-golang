...
[![Docker](https://img.shields.io/badge/Docker-v2.1.0.3-orange)](https://www.docker.com/)
...
6. Folder `3. ServiceDocker` for solution of question no. 3
   Before run this program, make sure CMD or terminal is in this folder directory.
   First, you need to build a docker image with command `docker build --tag (your-image-name) .`.
   Then, create a container from your docker image with command `docker container create --name (your-container-name) -p 8080:8080 (your-image-name)`.
   To start your container, write command `docker container start (your-container-name)`.
   Check if your container runs with `docker container ls`, and if it's run, it will be on the list.
   The server runs in `localhost:9000`.
   To stop your container, write command `docker container stop (your-container-name)`.
